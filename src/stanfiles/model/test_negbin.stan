...
functions {
  #include "/src/stanfiles/functions/generic_functions.stan"
  #include "/src/stanfiles/functions/spline_functions.stan"
  #include "/src/stanfiles/functions/SEIR_ode_spline.stan"
  #include "/src/stanfiles/functions/prevalence_noS.stan"
}
...
data {
  ...
  real p_phi; // prior for dispersion parameter of the negative binomial
...
transformed data {
  ...
  #include "/src/stanfiles/data/splines_transformed_data.stan"
...
parameters {
  ...
  real<lower=0> phi_inv;                  // create negative binomial for all models --> move to generic part
  //real kappa;                           // to prevent overfitting
...
model {
  ...
  // quasi poisson model
  //if (inference==1) {
  //  target += neg_binomial_2_lpmf( data_pre | incidence, to_array_1d( to_row_vector(incidence)/(theta-1)) );
  //}
