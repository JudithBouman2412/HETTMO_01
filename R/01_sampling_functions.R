...
standata <- function( data,
  ...
  priors <- list(p_I0 = c(1,2), # means that initially between
                 p_R0 = c(2,1))
...
create_function_initial_values <- function(data_list,
  ...
  initial_value_list$fraction_pre=runif(1,0.4,0.6)

  if(homogeneous){
    # General parameters
    initial_value_list$R0=rgamma(1,data_list$p_R0[1],data_list$p_R0[2])
    initial_value_list$I0_raw=rgamma(1,data_list$p_I0[1])
    initial_value_list$p_detect2 = rtruncnorm(1,a=0, b=1, 0.5, 0.1)

    # parameters based on method of time-dependence
    if (type=="spline"){
      initial_value_list$a_raw=rtruncnorm(data_list$num_knots+1, a = 0, b = Inf,
                                          ( data_list$p_R0[1]*(1/(data_list$generation_time*(0.5)))/data_list$contact )/data_list$beta_fixed, 1)
    } else if (type=="GP"){

    } else if (type=="BM"){
      initial_value_list$sigmaBM=rtruncnorm(1, 0, a=0, b=Inf, data_list$p_sigma_BM)
      initial_value_list$eta_noise=rnorm(data_list$num_t,0,1)
    }
  } else {
    initial_value_list$beta=rep(rtruncnorm(1,0,1,data_list$p_beta[1], data_list$p_beta[2]),3)
    initial_value_list$I0_raw=rep(rgamma(1,data_list$p_I0[1]),3)
    initial_value_list$a_raw=rbind(rtruncnorm(data_list$num_knots+2, a=0, b = Inf, ( data_list$p_beta[1]), 0.1) ,
                rtruncnorm(data_list$num_knots+2, a=0, b = Inf, ( data_list$p_beta[1]), 0.1) ,
                rtruncnorm(data_list$num_knots+2, a=0, b = Inf, ( data_list$p_beta[1]), 0.1) )
    initial_value_list$p_detect2 = rtruncnorm(3, a=0, b=1, 0.5, 0.1)
  }

  # parameters specific for sampling distribution
  if (sampling=="qp"){
    initial_value_list$theta=rtruncnorm(1, a = 1.5, b=Inf, data_list$p_theta[1], data_list$p_theta[2])
  } else if ( sampling =="negbin"){
...
sampling_model <- function( data_list,
  ...
  model_compiled = cmdstanr::cmdstan_model(stan_file = paste("/Users/jb22m516/Documents/GitHub/HETTMO/inst/stan/", model, sep =""),
                                        include_paths = "/Users/jb22m516/Documents/GitHub/HETTMO/inst/stan",
