...
#' @param degree degree of the polynomial of the spline
  ...
  for (i in 1:num_basis){
    for (j in 1:num_interval){
      if (support_bs[i,j]){
        ...
        b.hat[i,((j-1)*order+1):(j*order)] <- coef(lm( B[i,t_support]~ int_knots[t_support] + I(int_knots[t_support]^2)+ I(int_knots[t_support]^3))) #extend to use any degree/order
      }
    }
  }
